<x-layout>
	...
	<div class="bg-white p-8 rounded-lg shadow-md w-full">
		...
		<form action="{{route('profile.update')}}" method="POST" enctype="multipart/form-data">
			...
			<button class="w-full bg-green-500 text-white px-4 py-2 border-rounded hover:bg-green-600 focus:outline-none" type="submit">Save</button>
		</form>
	</div>
	...
	<div class="bg-white p-8 rounded-lg shadow-md w-full">
		...
		<div class="mt-4 bg-gray-100 p-4 rounded-lg mb-[40px]">
			<h4 class="text-lg mb-2 font-semibold">Applicants</h4>
			@forelse($job->applicants as $applicant)
				<div class="py-2">
					...
					<form action="{{route('applicant.destroy',$applicant->id)}}" onsubmit="return confirm('Are you sure you want to delete this applicant?')" method="POST">
						@csrf
						@method('DELETE')
						<button class="text-red-500 text-sm hover:text-red-700" type="submit"><i class="fas fa-trash"></i> Delete Applicant</button>
					</form>
				</div>
			@empty
			<p class="text-gray-700 mb-5">No applicants for this jobs</p>
			@endforelse
		</div>
		@empty 
